...
class GFBDatabaseController():
	...
	def addUser(self, email, password, firstName, lastName, credentials, hostSite, phoneNumber=None):
		...
		info = (email,password,firstName,lastName,credentials,phoneNumber,hostSite)
		try:
	...
	def changePassword(self, email, oldPass, newPass):
		q = "UPDATE Users SET password=%s WHERE email=%s AND password=%s" 
		info = (newPass, email, oldPass)
		try:
			self.cursor.execute(q, info)
	...
	def updateUser(self, email, newEmail, newPassword, newFirstName, newLastName, newPhoneNumber, hostSite, credentials):
		q = "UPDATE Users SET email=%s, password=%s, first_name=%s, last_name=%s, phone_number=%s, fk_credentials=%s, fk_hostsite_id=%s WHERE email=%s" 
		info = (newEmail, newPassword, newFirstName, newLastName, newPhoneNumber, credentials,hostSite, email)
		try:
			self.cursor.execute(q, info)
			result = self.cursor.rowcount
			self.cursor.fetchone()
	...
	def addHostSite(self, name, address, city, province, postalCode, phoneNumber, coordinatorIDs, hoursofOperation):
		...
		try:
			...
			return False
		q = "INSERT INTO HostSites (name,address,city,province,postal_code,phone_number,hours_id) VALUES (%s,%s,%s,%s,%s,%s,%s)" 
		info = (name,address,city,province,postalCode,phoneNumber,hours_id)
		try:
			self.cursor.execute(q, info)
			self.cnx.commit()
		except Exception as e:
	...
	def updateHostSite(self, hostSiteID, name, address, city, province, postalCode, phoneNumber, coordinatorIDs, hoursofOperation):
		...
		self.cursor.execute(q, info)
		hours_id = self.cursor.lastrowid
		q = "UPDATE HostSites SET name=%s, address=%s, city=%s, province=%s, postal_code=%s, hours_id=%s, phone_number=%s WHERE id=%s" 
